...
const navItemInitFunctions = {
  reviews: () => loadSiteReviews(currentURL, reviewsSection, reviews, null, null, $(`<span class="empty-label">No reviews found. Want to <span class="link-suggestion page-director" data-key="edit-review">create one</span>?</span>`)),
  "your-reviews": loadUserReviews,
  explorer: () =>
    getURLResults(currentURL)
};
...
function handleLoginAttempt() {
  ...
  if (username.trim().length == 0) {
    loginForm.username.next(".error-label").text("Don't forget your username!");
    shouldHideUsernameError = false;
  }
  if (shouldHideUsernameError) {
    loginForm.username.next(".error-label").text("");
  }
      ...
      $.ajax({
        ...
        error: function (data) {
          ...
          try {
            ...
            showErrorMessage(
              "An error occurred. Try closing/reopening this extension",
              5000
            );
          }
        },
...
function handleRegisterAttempt() {
  ...
  if (username.trim().length == 0) {
    ...
    registerForm.username
      .next(".error-label")
      .html(`Your username must be at<br/> least 4 characters`);
    shouldHideUsernameError = false;
...
stars.on("click", function () {
  const prev = $(this).html("star").addClass("selected").prevAll();
...
$("#edit-review #submit-review").on("click", function () {
  ...
  const activePrivacyToggle = $(
    "#edit-review #review-footer .privacy-toggle .privacy-option.active"
  );
  if (selectedStars == 0) {
    ...
    reviewErrorLabel.text(
      "Please choose a privacy option for your review below"
    );
    shouldHideReviewError = false;
  }
...
addTagInput.on("input", function () {
  ...
  if (query.length != 0) {
    ...
    tagSuggestions.html(
      '<span class="input-suggestion-label">Loading...</span>'
    );
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.data.length == 0) {
            tagSuggestions.html(
              '<span class="input-suggestion-label">No suggestions. Want to <span class="link-suggestion">create it</span>?</span>'
            );
          } else {
            let displayedTags = 0;
            data.data.forEach((e) => {
              let isUnique = true;
              const samePrivacyTagsAdded = reviewTags
                .find(`.tag[data-private=${e.private}]`)
                .toArray();
              ...
              if (isUnique) {
                tagSuggestions.append(
                  createTagElement(e.name, e.color, e.private, e.id)
                );
                displayedTags++;
              }
            });
            if (displayedTags == 0) {
              tagSuggestions.html(
                '<span class="input-suggestion-label">No suggestions.</span>'
              );
            }
          }
        },
...
function createTagElement(name, color, isPrivate, id) {
  const tagElem = $(
    `<div class="tag" data-private=${isPrivate}><span class="material-icons">${
    isPrivate ? "visibility_off" : "visibility"
    }</span><span class="tag-name">${name}</span></div>`
  );
...
$(document).on(
  "click",
  "#edit-review #tag-input-suggestions .link-suggestion",
  function (e) {
    newTagPopup.show();
...
$("#new-tag #new-tag-popup #new-tag-popup-colors .new-tag-color").each(
  function () {
    $(this).css("background-color", $(this).data("color"));
  }
);

const tagColorActiveIndicator = $(
  `<div class="active-indicator"><span class="material-icons">check</span></div>`
);
...
$("#new-tag #new-tag-popup #new-tag-popup-colors .new-tag-color").on(
  "click",
  function () {
...
$("#new-tag-button").on("click", function () {
  ...
  if (newTagName.trim().length == 0) {
    ...
    !tagColorActiveIndicator.is(":visible") &&
    customHex.trim().length == 0
  ) {
    ...
    setTimeout(function () {
      $.ajax({
        ...
        data: {
          ...
          isPrivate:
            $("#new-tag-popup .privacy-option.active").data("def") == "private",
        },
        headers: {
...
const ratingAverageLabel = $(
  "#reviews .rating-container .rating .rating-value"
);
  ...
  setTimeout(function () {
    ...
    $.ajax({
      ...
      error: function (data) {
        finishLoading();
        showErrorMessage(
          "Failed to load website reviews. Try closing & reopening this extension",
          10000
        );
        if (callbackOnFinish) {
          callbackOnFinish(false, data.data);
        }
      },
    });
  }, 500);
...
const websiteReviewStars = $(
  "#reviews .rating-container .rating .rating-stars .material-icons"
);

function processReviewsMetadata(data, container) {
  const totalReviews = data.totalReviews;
  $(".reviews-count", container).text(
    conditionallyPlurify(totalReviews, "review")
  );
  ...
  ratingBarPropNames.forEach((prop) => {
    ...
    $(
      `.rating-container .rating-distribution .rating-bar[data-def="${prop}"] > div`,
      container
    ).css("width", barWidth + "%");
  });
  ...
  const firstFullStars = $(
    ".rating .rating-container .rating-stars .material-icons",
    container
  ).slice(0, numOfFullStars);
...
function processReviewsData(data, container, noReviewsLabelElement, includeReviewURL) {
  ...
  data.forEach((review) => {
    ...
    const reviewDateElement = reviewElement.find(
      ".review-metadata .review-date"
    );
    ...
    review.tags.forEach((tag) => {
      reviewTagsContainer.append(
        createTagElement(tag.name, tag.color, tag.private, tag.id)
      );
    });

    reviewElement
      .find(".review-stars .material-icons")
...
userReviewsSortOptions.on("click", function () {
  ...
  const sortOptionText = sortOption
    .find(".expandable-option-label")
    .text()
    .trim();
...
const userReviewsTagFilterPopup = $(
  "#your-reviews #your-reviews-tag-filter-popup"
);
...
function renderTagsDropdown(data) {
  ...
  const allTagsFilter = $(
    `<div class="expandable-option"><span class="material-icons">check</span><div class="expandable-option-label"><span class="expandable-option-label-text">None</span></div></div>`
  );
  ...
  Object.values(tags).forEach((tag) => {
    const tagElem = $(
      `<div class="expandable-option" data-def="recent" data-id="${
      tag.id
      }"><span class="material-icons">check</span><div class="expandable-option-label"><span class="material-icons">${
      tag.private ? "visibility_off" : "visibility"
      }</span><span class="expandable-option-label-text"></span></div></div>`
    );
...
$(userReviewsTagFilterPopup).on("click", ".expandable-option", function () {
  ...
  loadUserReviews(
    ...
    function (didSucceed) {
      if (didSucceed) {
        userReviewsTagFilterPopup
          .find(".expandable-option > .material-icons")
          .hide();
        if (!selectedTagFilterId) {
          userReviewsTagFilterPopup
            .find(".expandable-option")
            .first()
            .find(".material-icons")
            .show();
        } else {
          userReviewsTagFilterPopup
            .find(`.expandable-option[data-id="${id}"] .material-icons`)
            .show();
        }
      }
    }
...
urlBar.on("click", function () {
  if (urlBar.prop("contenteditable") != "true") {
    ...
    const urlBarRange = document.createRange();
    urlBarRange.selectNodeContents(urlBar[0]);
    selection.addRange(urlBarRange);
  }
});
...
function parseURLIntoChunks(url) {
  try {
    ...
    return [
      parsedURL.protocol,
      parsedURL.hostname,
      ...parsedURL.pathname.split("/").filter((e) => e),
    ];
  } catch (err) {
    showErrorMessage('Explorer: Invalid URL', 5000)
    return null;
  }
}
...
function convertChunksToElems(chunks) {
  chunks.forEach((chunk, index) => {
    const chunkElem = $(
      `<div class="url-part"><div class="url-part-label"></div><span class="material-icons">arrow_right</span></div>`
    );
...
const explorerReviewsSortPopup = $('#explorer #explorer-reviews-sort #explorer-reviews-sort-popup')
const explorerReviewsSortOptions = $('.expandable-option', explorerReviewsSortPopup)

explorerReviewsSortOptions.on('click', function (e) {
...
$(document).on("click", function (e) {
  ...
  if (!e.target.closest('#explorer-reviews-sort-popup')) {
    closeExpandablePopup(explorerReviewsSortPopup)
  }
});
...
$(document).on("click", ".expandable-popup-trigger", function (e) {
  ...
  const [desiredWidth, desiredHeight] = [
    popupElement.outerWidth(),
    popupElement.outerHeight(),
  ];
  popupElement.hide();
...
const notificationElement = $(
  `<div class="notification"><div class="icon"><span class="material-icons type-icon"></span></div><div class="message"></div><span class="material-icons notification-close">close</span></div>`
);
